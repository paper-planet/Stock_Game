...
class OrderBook:
    def __init__(self,asset,seed=0,levels=10):self.asset=asset;self.rng=random.Random(seed+hash(asset.symbol)%100000);self.levels=levels;self.bids=[];self.asks=[];self.last_trade=asset.price;self.trade_size=100;self.imbalance=0.;self.update()
    def update(self,mid=None,pressure=0.,vol_mult=1.):
        p=max(.0001,float(mid if mid is not None else self.asset.price));tick=max(.0001,p*.00003);base=max(50,int(1800*max(.15,min(4,vol_mult))*(1+abs(pressure))));self.imbalance=max(-1,min(1,pressure));self.bids=[];self.asks=[];makers=('VIRTEX','ALPHA_MM','NOVA_MM','FLOW_MM','CROSSING')
        for i in range(1,self.levels+1):
            dist=tick*i*(1+self.rng.random()*.8);skew=pressure*p*.0007;bp=max(.0001,p-dist+skew);ap=max(.0001,p+dist+skew);bs=max(1,int(base/(i**.60)*self.rng.uniform(.55,1.55)));asz=max(1,int(base/(i**.60)*self.rng.uniform(.55,1.55)));self.bids.append(BookLevel(bp,bs,self.rng.randint(1,max(2,min(99,bs//8))),self.rng.choice(makers),self.rng.choice(makers),int(bs*.1)));self.asks.append(BookLevel(ap,asz,self.rng.randint(1,max(2,min(99,asz//8))),self.rng.choice(makers),self.rng.choice(makers),int(asz*.1)))
