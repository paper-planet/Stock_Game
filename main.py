...
def main_menu(root,accounts):
    result={'action':'cancel','mode':'MEDIUM','cash':250000,'username':None};w=tk.Toplevel(root);w.title('STOCK_GAME PRO — MAIN MENU');w.geometry('760x620+100+60');w.configure(bg='#071019');w.protocol('WM_DELETE_WINDOW',w.destroy);w.deiconify();w.lift();w.attributes('-topmost',True);w.after(250,lambda: w.attributes('-topmost',False));w.focus_force()
...
def main():
    try:
        ...
        _Path(__file__).with_name('startup_error.log').unlink(missing_ok=True)
    except Exception:
        pass
...
def _write_startup_error(exc):
    import traceback
    try:
        p = __import__('pathlib').Path(__file__).with_name('startup_error.log')
...
if __name__=='__main__':
    try:
        ...
        try:
            ...
            mb.showerror('StockGame Pro failed to start',
                         f'{type(exc).__name__}: {exc}\n\n'
                         'A full traceback was saved to startup_error.log')
        except Exception:
            pass
        raise
